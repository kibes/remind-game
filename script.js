...
function initSnowSystem() {
    ...
    // Настройки снега
    // Настройки снега (обновленные)
        const settings = {
            color: "245, 255, 255", 
            minRadius: 2.0,         // Увеличили минимальный размер (было 1)
            maxRadius: 8.0,         // Увеличили максимальный размер (было 3.5)
            minSpeed: 0.5,          // Замедлили минимальную скорость (было 0.5)
            maxSpeed: 1.5,          // Замедлили максимальную скорость (было 2.5)
            density: 10000           // Немного увеличили делитель, чтобы крупных снежинок 
                                    // не было слишком много (чтобы не перекрывали обзор)
        };
    ...
    function resize() {
        ...
        // КЛЮЧЕВОЙ МОМЕНТ: Расчет количества от ПЛОЩАДИ экрана.
        // Это решает проблему "мало снега" в горизонтальном режиме.
        const area = width * height;
        const targetCount = Math.floor(area / settings.density);
        
        // Пересоздаем снежинки при ресайзе, чтобы их было нужное количество
        snowflakes = [];
    ...
    function createSnowflake(isNew = true) {
        return {
            x: Math.random() * width,
            y: isNew ? Math.random() * height : -10, // Если создаем с нуля - раскидываем по экрану, если респавн - сверху
            ...
            sway: Math.random() * 0.02 - 0.01,  // Покачивание влево-вправо
            swayOffset: Math.random() * Math.PI * 2 // Фаза покачивания
        };
    }
    ...
    function update() {
        ...
        snowflakes.forEach((flake) => {
            ...
            flake.x += Math.sin(flake.swayOffset) * 0.5; // Плавное покачивание

            // Отрисовка
            ctx.beginPath();
            ctx.arc(flake.x, flake.y, flake.radius, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(${settings.color}, ${flake.opacity})`;
            ctx.fill();

            // Если улетела вниз - возвращаем наверх
            if (flake.y > height) {
                Object.assign(flake, createSnowflake(false));
            }
            // Если улетела вбок - возвращаем с другой стороны
    ...
    update(); // Запуск анимации
}

// Добавляем инициализацию снега в window.onload
const originalOnLoad = window.onload;
window.onload = async () => {
    if (originalOnLoad) await originalOnLoad(); // Выполняем старый onload (игра)
    initSnowSystem(); // Запускаем снег
};
